def ler_numero(prompt):
    while True:
        try:
            ...
            print("Erro! O caractere digitado não é um número. Por favor, tente novamente.")
    ...
    while True:
        ...
        try:
            ...
            if resultado % 1 == 0:
                ...
                if (input("Deseja converter o resultado para decimal? (s/n): ").strip().lower() == 's'):
                    print(f"O resultado da operação em decimal é: {format(float(resultado), '.2f')}")
            else:
                print(f"O resultado da operação é: {format(resultado, '.2f')}")
                if (input("Deseja converter o resultado para inteiro? (s/n): ").strip().lower() == 's'):
                    print(f"O resultado da operação em inteiro é: {int(float(resultado))}")
